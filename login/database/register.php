...
if (isset($_POST['submit1'])) {
	...
	if ($password == $cpassword) {
		$sql = "SELECT * FROM lc_user WHERE email='$email'";
		$result = mysqli_query($conn, $sql);

	if (!$result->num_rows > 0) {
		$sql = "INSERT INTO lc_user (fullname, username, email, phone, password) VALUES ('$fullname', '$username', '$email', '$phone', '$password')";
		$result = mysqli_query($conn, $sql);

	if (!$result) {
				echo "<script>alert('Something Went Wrong.')</script>";echo "<script>alert('Registration Completed.')</script>";
				$fullname = "";
				$username = "";
				$email = "";
				$phone = "";
